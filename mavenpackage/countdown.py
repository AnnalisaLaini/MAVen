...
def countdown(date_str):
    # we import the date class from the datetime module and
...
# we transform the string inputted by the user to a
# datetime object and we consider only the date part
date_obj = datetime.datetime.strptime(date_str, '%Y-%m-%d').date()

# we compute the difference in days between the date
# inputted by the user and the current local date
diff = (date_obj - today).days

# we create different cases
if diff > 0:
    # if the difference is bigger than 0,
    # we print the number of days left
    print("Time left: {} days".format(diff))

elif diff == 0:
    # if the difference is equal to 0,
    # we say to the user that the input date is the current date
    print("Time left: {} days!\
    The date you input refers to today".format(diff))

else:
    # if the difference is smaller than 0,
    # we take the absolute value of the difference in days and we
    # print the number of days passed since that day
    diff = abs(diff)
